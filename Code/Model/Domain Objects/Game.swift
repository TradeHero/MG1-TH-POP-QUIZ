...
class Game: JSONDecodable, DebugPrintable, Equatable, DictionaryRepresentation {
    ...
    init(id: Int, challenger: User, opponentUser: User, rejectedByOpponent: Bool, expired: Bool, questionSet: [Question], challengerResult: GameResult?, opponentResult: GameResult?, createdAt: NSDate!) {
        self.id = id
    ...
    class func create(id: Int)(challenger: User)(createdAtUtc: String)(opponent: User)(questionSet: [Question])(challengerResult: GameResult?)(opponentResult: GameResult?)(rejectedByOpponent: Bool)(expired: Bool) -> Game {
        var date = DataFormatter.shared.dateFormatter.dateFromString(createdAtUtc)
        var selfUser: User
        var awayUser: User


        return Game(id: id, challenger: challenger, opponentUser: opponent, rejectedByOpponent: rejectedByOpponent, expired: expired, questionSet: questionSet, challengerResult: challengerResult, opponentResult: opponentResult, createdAt: date)
    }
